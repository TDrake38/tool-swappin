...
Messages.getMessagesByUserId = async (id, params) => {
  const findMessage = await queryDB(
    "SELECT * FROM user_messages WHERE (id_sender = $1 AND id_recipient = $2) OR ( id_recipient = $1 AND id_sender = $2)",
    [id, params]
  );
...
Messages.getById = async (messageID) => {
  const oneMessage = await queryDB(
...
Messages.createMessage = async (message, id, params) => {
  const createMessage = await queryDB(
    "INSERT INTO user_messages (message, id_sender, id_recipient, sent) VALUES ($1, $2, $3, CURRENT_TIMESTAMP) RETURNING * ",
    [message, id, params]
  );
...
Messages.deleteConversation = async (id, params) => {
  const deleteMessage = await queryDB(
    "DELETE FROM user_messages WHERE (id_sender = $1 AND id_recipient = $2) OR ( id_recipient = $1 AND id_sender = $2) RETURNING * ",
    [id, params]
  );
