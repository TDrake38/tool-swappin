...
Messages.getMessagesByUserIdTest = async (id) => {
    const findMessage = await queryDB('SELECT * FROM user_messages WHERE id_sender = $1 AND id_recipient = ${id} ORDER BY sent ASC|DESC', [id]);
    return findMessage.rows;
}
